...
gcloud run deploy "$SERVICO_NOME" \
    ...
    --min-instances 0 \
    --max-instances 3 \
    --set-env-vars="AMB=${AMBIENTE},EXIGE_AUTH=1,HTTPS_ONLY=1,GOOGLE_CLIENT_ID=${GOOGLE_CLIENT_ID},GOOGLE_CLIENT_SECRET=${GOOGLE_CLIENT_SECRET},SESSION_SECRET_KEY=${SESSION_SECRET_KEY},EMAILS_PERMITIDOS=${EMAILS_PERMITIDOS}"
